...
public class ClientKitchenTypeController {

    private final ClientKitchenTypeService service;
    private final MessageSource messageSource;

    @PostMapping("kitchen-type/{typeId}")
    public ResponseEntity<GeneralBodyResponse<ClientKitchenTypeDto>> save(@AuthenticationPrincipal UserEntity loggedUser, @PathVariable("typeId") UUID typeId, Locale locale) {
    ...
    public ResponseEntity<GeneralBodyResponse<List<ClientKitchenTypeDto>>> getAll(@AuthenticationPrincipal UserEntity loggedUser, Locale locale) {
        return ResponseEntity
                .status(HttpStatus.OK)
